package com.zii.school.model

import android.os.Parcelable
import kotlinx.parcelize.Parcelize
import com.zii.school.util.*

/**
 * Identity announcement structure with TLV encoding
 * Compatible with iOS AnnouncementPacket TLV format
 * 
 * Now includes optional friendCode for friend verification system.
 */
@Parcelize
data class IdentityAnnouncement(
    ...
    val friendCode: String? = null    // Optional: Zii friend code for friend verification
) : Parcelable {
    ...
    private enum class TLVType(val value: UByte) {
        ...
        FRIEND_CODE(0x04u);  // NEW: Zii friend code for friend system
    ...
    fun encode(): ByteArray? {
        ...
        val friendCodeData = friendCode?.toByteArray(Charsets.UTF_8)
        
        // Check size limits
        if (nicknameData.size > 255 || noisePublicKey.size > 255 || signingPublicKey.size > 255) {
            return null
        }
        if (friendCodeData != null && friendCodeData.size > 255) {
            return null
        }
        ...
        // TLV for friend code (optional)
        if (friendCodeData != null) {
            result.add(TLVType.FRIEND_CODE.value.toByte())
            result.add(friendCodeData.size.toByte())
            result.addAll(friendCodeData.toList())
        }
        ...
        fun decode(data: ByteArray): IdentityAnnouncement? {
            ...
            while (offset + 2 <= dataCopy.size) {
                ...
                when (type) {
                    ...
                    TLVType.FRIEND_CODE -> {
                        friendCode = String(value, Charsets.UTF_8)
                    }
                    null -> {
            ...
            // All three core fields are required, friendCode is optional
            return if (nickname != null && noisePublicKey != null && signingPublicKey != null) {
                IdentityAnnouncement(nickname, noisePublicKey, signingPublicKey, friendCode)
            } else {
                null
            }
        }
