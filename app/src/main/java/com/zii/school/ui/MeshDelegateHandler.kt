...
import com.zii.school.ui.NotificationTextUtils
...
class MeshDelegateHandler(
    ...
    override fun didUpdatePeerList(peers: List<String>) {
        coroutineScope.launch {
            ...
            runCatching { com.zii.school.services.MessageRouter.tryGetInstance()?.onPeersUpdated(peers) }
            ...
            state.getSelectedPrivateChatPeerValue()?.let { currentPeer ->
                ...
                // Simplified: No Nostr/geohash migration logic needed for BT mesh only
                if (isMeshEphemeral && !peers.contains(currentPeer)) {
                    // Mesh chat lost connection - just cleanup
                    privateChatManager.cleanupDisconnectedPeer(currentPeer)
                }
            }

            // Simplified: No Nostr unification needed for BT mesh only
        }
    }
     ...
     */
    fun getPeerInfo(peerID: String): com.zii.school.mesh.PeerInfo? {
