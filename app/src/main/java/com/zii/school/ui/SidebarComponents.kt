...
import com.zii.school.ui.theme.BASE_FONT_SIZE
            ...
            Row(
                ...
                IconButton(
                    ...
                    Icon(
                        ...
                        contentDescription = stringResource(com.zii.school.R.string.cd_leave_channel),
...
fun PeopleSection(
    ...
    Column(modifier = modifier) {
        ...
        // Build mapping of connected peerID -> noise key hex (BT mesh only)
        ...
        // Build a map of base name counts across all people shown in the list (connected + offline)
        ...
        sortedPeers.forEach { pid ->
            ...
            val (b, _) = com.zii.school.ui.splitSuffix(dn)
            if (b != "You") baseNameCounts[b] = (baseNameCounts[b] ?: 0) + 1
        }

        // Offline favorites (exclude ones mapped to connected) - BT mesh only
        val offlineFavorites = com.zii.school.favorites.FavoritesPersistenceService.shared.getOurFavorites()
        ...
        sortedPeers.forEach { peerID ->
            ...
            val combinedHasUnread = meshUnread
            val combinedUnreadCount = privateChats[peerID]?.count { msg -> msg.sender != nickname && meshUnread } ?: 0

            val displayName = if (peerID == nickname) "You" else (peerNicknames[peerID] ?: (privateChats[peerID]?.lastOrNull()?.sender ?: peerID.take(12)))
            val (bName, _) = com.zii.school.ui.splitSuffix(displayName)
        ...
        // Append offline favorites (not currently connected) - BT mesh only
        ...
        offlineFavorites.forEach { fav ->
            ...
            val dn = peerNicknames[favPeerID] ?: fav.peerNickname
            val (bName, _) = com.zii.school.ui.splitSuffix(dn)
            val showHash = (baseNameCounts[bName] ?: 0) > 1

            // Compute unreadCount from noise conversation only (BT mesh)
            ...
            PeerItem(
                ...
                showNostrGlobe = false, // BT mesh only
                showHashSuffix = showHash
            )
...
private fun PeerItem(
    ...
    val (baseNameRaw, suffixRaw) = com.zii.school.ui.splitSuffix(displayName)
    ...
    Row(
        ...
        if (hasUnreadDM) {
            ...
            Icon(
                ...
                contentDescription = stringResource(com.zii.school.R.string.cd_unread_message),
                modifier = Modifier.size(16.dp),
                tint = Color(0xFFFF9500) // iOS orange
            )
        } else {
            // Connection indicator icons (BT mesh only)
            if (!isDirect && isFavorite) {
                // Offline favorited user: show outlined circle icon
                Icon(
                    imageVector = Icons.Outlined.Circle,
                    contentDescription = stringResource(com.zii.school.R.string.cd_offline_favorite),
                    modifier = Modifier.size(16.dp),
                    tint = Color.Gray
                )
            } else {
