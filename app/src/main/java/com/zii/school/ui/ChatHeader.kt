...
import com.zii.school.core.ui.utils.singleOrTripleClickable
...
fun PeerCounter(
    ...
    isConnected: Boolean,
    selectedLocationChannel: com.zii.school.geohash.ChannelID?,
    ...
    val (peopleCount, countColor) = when (selectedLocationChannel) {
        is com.zii.school.geohash.ChannelID.Location -> {
        ...
        is com.zii.school.geohash.ChannelID.Mesh,
        null -> {
    ...
    Row(
        ...
        Icon(
            ...
            contentDescription = when (selectedLocationChannel) {
                is com.zii.school.geohash.ChannelID.Location -> stringResource(R.string.cd_geohash_participants)
...
fun ChatHeaderContent(
    ...
    onboardingTargets: com.zii.school.onboarding.OnboardingTargets? = null
) {
    val colorScheme = MaterialTheme.colorScheme
    ...
    val isMutualFavorite = remember(peerID, peerNicknames) {
        try {
            ...
            if (peerID.length == 64 && peerID.matches(Regex("^[0-9a-fA-F]+$"))) {
                ...
                com.zii.school.favorites.FavoritesPersistenceService.shared.getFavoriteStatus(noiseKeyBytes)?.isMutual == true
            } else if (peerID.length == 16 && peerID.matches(Regex("^[0-9a-fA-F]+$"))) {
                com.zii.school.favorites.FavoritesPersistenceService.shared.getFavoriteStatus(peerID)?.isMutual == true
            } else false
        } catch (_: Exception) { false }
    }

    // Geohash DM title removed - BT mesh only
    val titleText: String = peerNicknames[peerID] ?: run {
        val titleFromFavorites = try {
            if (peerID.length == 64 && peerID.matches(Regex("^[0-9a-fA-F]+$"))) {
                val noiseKeyBytes = peerID.chunked(2).map { it.toInt(16).toByte() }.toByteArray()
                com.zii.school.favorites.FavoritesPersistenceService.shared.getFavoriteStatus(noiseKeyBytes)?.peerNickname
            } else if (peerID.length == 16 && peerID.matches(Regex("^[0-9a-fA-F]+$"))) {
                com.zii.school.favorites.FavoritesPersistenceService.shared.getFavoriteStatus(peerID)?.peerNickname
            } else null
...
private fun MainHeader(
    ...
    val bookmarksStore = remember { com.zii.school.geohash.GeohashBookmarksStore.getInstance(context) }
        ...
        Row(
            ...
            Row(verticalAlignment = Alignment.CenterVertically, modifier = Modifier.padding(end = 4.dp)) {
                ...
                val currentGeohash: String? = when (val sc = selectedLocationChannel) {
                    is com.zii.school.geohash.ChannelID.Location -> sc.channel.geohash
                    else -> null
                }
            ...
            // LocationNotesButton removed - BT mesh only

            // Tor removed - BT mesh only
            
            // PoWStatusIndicator removed - BT mesh only
...
private fun LocationChannelsButton(
    ...
    val (badgeText, badgeColor) = when (selectedChannel) {
        ...
        is com.zii.school.geohash.ChannelID.Location -> {
            val geohash = (selectedChannel as com.zii.school.geohash.ChannelID.Location).channel.geohash
