package com.zii.school.protocol
...
enum class MessageType(val value: UByte) {
    ...
    WIFI_HANDSHAKE_REQUEST(0x30u), // WiFi friend discovery request (broadcast friend code)
    WIFI_HANDSHAKE_RESPONSE(0x31u); // WiFi friend discovery response (send connection info)
...
data class BitchatPacket(
    ...
    fun toBinaryDataForSigning(): ByteArray? {
        ...
        val unsignedPacket = BitchatPacket(
            ...
            ttl = com.zii.school.util.AppConstants.SYNC_TTL_HOPS // Use fixed TTL=0 for signing to ensure relay compatibility
        )
