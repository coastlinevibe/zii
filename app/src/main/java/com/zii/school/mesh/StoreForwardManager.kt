...
class StoreForwardManager {
    ...
    companion object {
        ...
        private const val MESSAGE_CACHE_TIMEOUT = com.zii.school.util.AppConstants.StoreForward.MESSAGE_CACHE_TIMEOUT_MS  // 12 hours for regular peers
        private const val MAX_CACHED_MESSAGES = com.zii.school.util.AppConstants.StoreForward.MAX_CACHED_MESSAGES  // For regular peers
        private const val MAX_CACHED_MESSAGES_FAVORITES = com.zii.school.util.AppConstants.StoreForward.MAX_CACHED_MESSAGES_FAVORITES  // For favorites
        private const val CLEANUP_INTERVAL = com.zii.school.util.AppConstants.StoreForward.CLEANUP_INTERVAL_MS // 10 minutes
    }
    ...
    fun cacheMessage(packet: BitchatPacket, messageID: String) {
        ...
        // Schedule auto-deletion for cached messages based on auto-delete settings
        scheduleAutoDeleteForCachedMessage(messageID)
    }
    
    /**
     * Schedule auto-deletion for cached messages
     * Respects the global auto-delete settings
     */
    private fun scheduleAutoDeleteForCachedMessage(messageID: String) {
        // Auto-delete for cached messages uses the same timeout as MESSAGE_CACHE_TIMEOUT
        // This ensures cached messages don't persist longer than the auto-delete setting
        managerScope.launch {
            delay(MESSAGE_CACHE_TIMEOUT)
            
            // Remove from cache after timeout
            messageCache.removeAll { it.messageID == messageID }
            favoriteMessageQueue.values.forEach { queue ->
                queue.removeAll { it.messageID == messageID }
            }
            
            Log.d(TAG, "Auto-deleted cached message: ${messageID.take(8)}...")
        }
    }
