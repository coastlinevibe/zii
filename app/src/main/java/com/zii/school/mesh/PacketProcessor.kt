...
class PacketProcessor(private val myPeerID: String) {
    ...
    private suspend fun handleReceivedPacket(routed: RoutedPacket) {
        ...
        when (messageType) {
            ...
            MessageType.WIFI_HANDSHAKE_REQUEST -> handleWiFiHandshakeRequest(routed)
            MessageType.WIFI_HANDSHAKE_RESPONSE -> handleWiFiHandshakeResponse(routed)
            else -> {
    ...
    /**
     * Handle WiFi handshake request (broadcast)
     */
    private suspend fun handleWiFiHandshakeRequest(routed: RoutedPacket) {
        val peerID = routed.peerID ?: "unknown"
        Log.d(TAG, "Processing WiFi handshake request from ${formatPeerForLog(peerID)}")
        delegate?.handleWiFiHandshakeRequest(routed)
    }
    
    /**
     * Handle WiFi handshake response (direct)
     */
    private suspend fun handleWiFiHandshakeResponse(routed: RoutedPacket) {
        val peerID = routed.peerID ?: "unknown"
        Log.d(TAG, "Processing WiFi handshake response from ${formatPeerForLog(peerID)}")
        delegate?.handleWiFiHandshakeResponse(routed)
    }
...
interface PacketProcessorDelegate {
    ...
    fun handleWiFiHandshakeRequest(routed: RoutedPacket)
    fun handleWiFiHandshakeResponse(routed: RoutedPacket)
