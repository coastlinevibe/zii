...
class MessageHandler(private val myPeerID: String, private val appContext: android.content.Context) {
    ...
    suspend fun handleNoiseEncrypted(routed: RoutedPacket) {
        ...
        try {
            ...
            val noisePayload = com.zii.school.model.NoisePayload.decode(decryptedData)
            ...
            when (noisePayload.type) {
                com.zii.school.model.NoisePayloadType.PRIVATE_MESSAGE -> {
                    // Decode TLV private message exactly like iOS
                    val privateMessage = com.zii.school.model.PrivateMessagePacket.decode(noisePayload.data)
                ...
                com.zii.school.model.NoisePayloadType.FILE_TRANSFER -> {
                    // Handle encrypted file transfer; generate unique message ID
                    val file = com.zii.school.model.BitchatFilePacket.decode(noisePayload.data)
                    if (file != null) {
                        Log.d(TAG, "🔓 Decrypted encrypted file from $peerID: name='${file.fileName}', size=${file.fileSize}, mime='${file.mimeType}'")
                        val uniqueMsgId = java.util.UUID.randomUUID().toString().uppercase()
                        val savedPath = com.zii.school.features.file.FileUtils.saveIncomingFile(appContext, file)
                        val message = BitchatMessage(
                            id = uniqueMsgId,
                            sender = delegate?.getPeerNickname(peerID) ?: "Unknown",
                            content = savedPath,
                            type = com.zii.school.features.file.FileUtils.messageTypeForMime(file.mimeType),
                            timestamp = java.util.Date(packet.timestamp.toLong()),
                            isRelay = false,
                            isPrivate = true,
                            recipientNickname = delegate?.getMyNickname(),
                ...
                com.zii.school.model.NoisePayloadType.DELIVERED -> {
                ...
                com.zii.school.model.NoisePayloadType.READ_RECEIPT -> {
    ...
    private suspend fun sendDeliveryAck(messageID: String, senderPeerID: String) {
        try {
            ...
            val ackPayload = com.zii.school.model.NoisePayload(
                type = com.zii.school.model.NoisePayloadType.DELIVERED,
    ...
    suspend fun handleAnnounce(routed: RoutedPacket): Boolean {
        ...
        if (age > com.zii.school.util.AppConstants.Mesh.STALE_PEER_TIMEOUT_MS) {
            Log.w(TAG, "Ignoring stale ANNOUNCE from ${peerID.take(8)} (age=${age}ms > ${com.zii.school.util.AppConstants.Mesh.STALE_PEER_TIMEOUT_MS}ms)")
            return false
        }
        ...
        try {
            ...
            if (response != null) {
                ...
                val responsePacket = BitchatPacket(
                    ...
                    payload = response,
                    signature = null,
                    ttl = com.zii.school.util.AppConstants.MESSAGE_TTL_HOPS // Same TTL as iOS
                )
    ...
    private suspend fun handleBroadcastMessage(routed: RoutedPacket) {
        ...
        try {
            ...
            val isFileTransfer = com.zii.school.protocol.MessageType.fromValue(packet.type) == com.zii.school.protocol.MessageType.FILE_TRANSFER
            val file = com.zii.school.model.BitchatFilePacket.decode(packet.payload)
            if (file != null) {
                if (isFileTransfer) {
    ...
    private fun handleFavoriteNotificationFromMesh(content: String, fromPeerID: String) {
        try {
            ...
            if (noiseKey != null) {
                // Favorites removed - BT mesh only

                // Favorites removed - BT mesh only
                val rel: com.zii.school.favorites.FavoritesPersistenceService.FavoriteRelationship? = null
                val guidance = if (isFavorite) {
                    if (rel != null && rel.peerNoisePublicKey.isNotEmpty()) {
                ...
                val sys = com.zii.school.model.BitchatMessage(
                    sender = "system",
