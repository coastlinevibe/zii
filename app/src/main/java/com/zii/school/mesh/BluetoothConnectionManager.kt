...
class BluetoothConnectionManager(
    ...
    init {
        ...
        try {
            val dbg = com.zii.school.ui.debug.DebugSettingsManager.getInstance()
            // Role enable/disable
    ...
    fun startServices(): Boolean {
        ...
        // }
            ...
            connectionScope.launch {
                ...
                val dbg = try { com.zii.school.ui.debug.DebugSettingsManager.getInstance() } catch (_: Exception) { null }
    ...
    override fun onPowerModeChanged(newMode: PowerManager.PowerMode) {
        ...
        connectionScope.launch {
            ...
            val serverEnabled = try { com.zii.school.ui.debug.DebugSettingsManager.getInstance().gattServerEnabled.value } catch (_: Exception) { true }
            if (serverEnabled) {
            ...
            if (wasUsingDutyCycle != nowUsingDutyCycle) {
                ...
                val clientEnabled = try { com.zii.school.ui.debug.DebugSettingsManager.getInstance().gattClientEnabled.value } catch (_: Exception) { true }
                if (clientEnabled) {
            ...
            try {
                val maxServer = com.zii.school.ui.debug.DebugSettingsManager.getInstance().maxServerConnections.value
